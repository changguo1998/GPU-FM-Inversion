...
#
# Usage:
#   julia --project=input input/src/input.jl <raw.h5> <config.toml>
...
function main()
    if length(ARGS) < 2
        println(stderr, "Usage: julia --project=input input/src/input.jl <raw.h5> <config.toml>")
        exit(1)
    end
    ...
    # ── 2. Read raw.h5 ─────────────────────────────────────────────────────────
    event = read_event(raw_path)
    picks = read_phase_picks(raw_path)
    stations = read_stations(raw_path)
    ...
    for freq_idx in 1:n_frequencies
        ...
        if "PSR" in misfit_modules
            for (p_pid, s_pid) in psr_pairs
                ...
                p_wf = read_waveform(raw_path, p_pid)
                s_wf = read_waveform(raw_path, s_pid)
    ...
    write_database(db_path, greens, data, index, db_config)
    ...
    write_strategy(status_path, strategy)
