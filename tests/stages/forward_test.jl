...
function sdr_to_mt(s::Float64, d::Float64, r::Float64)::Vector{Float64}
    ...
    cr = cosd(r)
    return [
        -(sd * cr * sind(2s) + sind(2d) * sr * ss^2),  # Mxx
        sd * cr * sind(2s) - sind(2d) * sr * cs^2,     # Myy
        sind(2d) * sr,                                 # Mzz
        sd * cr * cosd(2s) + 0.5 * sind(2d) * sr * sind(2s),  # Mxy
        -(cd * cr * cs + cosd(2d) * sr * ss),          # Mxz
        -(cd * cr * ss - cosd(2d) * sr * cs),          # Myz
    ]
end
