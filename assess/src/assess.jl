...
# Load shared/local modules via include (not registered packages)
include(joinpath(@__DIR__, "..", "..", "shared", "AssessUtils.jl", "src", "AssessUtils.jl"))
using .AssessUtils: aggregate_misfits

include(joinpath(@__DIR__, "grid_refinement.jl"))
using .GridRefinement: refine_strategy, prompt_operator, TrialResult

function main()
    # ── Parse CLI ───────────────────────────────────────
    if length(ARGS) < 2
        println(stderr, "Usage: assess.jl <status_N.h5> <database.h5>")
        exit(1)
    end
    status_file = ARGS[1]
    database_file = ARGS[2]

    # ── Determine iteration N from filename ─────────────
    ...
    # Build output filename: status_N.h5 → status_{N+1}.h5
    next_status_file = joinpath(dirname(status_file), "status_$(n+1).h5")

    # ── Read inputs ─────────────────────────────────────
    trials = read_trials(status_file)
    misfits_map = read_misfits(status_file)
    strategy = read_strategy(status_file)
    config = read_config(database_file)

    # ── Validate required misfit keys ───────────────────
    ...
    # ── Convert Int32 masks to Bool vectors ─────────────
    ...
    # ── Set converged flag based on operator answer ─────
    if continue_flag
        # Rebuild strategy with converged=0
        ...
        # Rebuild strategy with converged=1
        next_strategy = Strategy(
            next_strategy.strike0, next_strategy.dstrike, next_strategy.nstrike,
            next_strategy.dip0, next_strategy.ddip, next_strategy.ndip,
            next_strategy.rake0, next_strategy.drake, next_strategy.nrake,
