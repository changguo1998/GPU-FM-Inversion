...
using HDF5IO: Strategy

# ─────────────────────────────────────────────────────────
# TrialResult — aggregated best-trial info for refinement
# ─────────────────────────────────────────────────────────
...
"""
    refine_strategy(current::Strategy, best_trial::TrialResult) -> Strategy
...
Returns a new `Strategy` with `converged=0` and `iteration` incremented.
The caller (assess.jl) prompts the operator and writes the strategy to
`status_{N+1}.h5`.
"""
function refine_strategy(current::Strategy, best_trial::TrialResult)
    ...
    return Strategy(
        ...
        copy(current.polarity_station_mask),
        copy(current.psr_station_mask),
...
"""
    prompt_operator(best_sdr, misfit, current::Strategy; io_in=stdin, io_out=stdout) -> Bool
...
"""
function prompt_operator(best_sdr, misfit, current::Strategy;
                         io_in::IO=stdin, io_out::IO=stdout)
    println(io_out)
