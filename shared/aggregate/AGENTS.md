...
Misfit aggregation: apply weights to raw misfit matrices, produce per-trial total scores, find best trial. All input phases/channels used equally — no channel selection. Also provides uncertainty computation helpers.
...
`aggregate_misfits(xcorr, polarity, psr, module_weights)`
...
1. Per-module sum — for each trial, sum non-NaN values across all rows. If no values contribute (all NaN), trial score = NaN for that module.
...
- NaN values in rows skipped (do not propagate to trial total)
