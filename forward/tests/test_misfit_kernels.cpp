...
static int test_polarity_all_sign_combos() {
    ...
    fm::launch_polarity_kernel<Backend::OpenMP>(mt.data(), pol_vec.data(), obs_pol.data(),
                                                misfit.data(), N_stations, N_trials);
    ...
    if (!std::isnan(misfit[0 + 3 * N_stations]) && misfit[0 + 3 * N_stations] != 1.0)
    ...
    // Station 4: expects -1
    if (!std::isnan(misfit[4 + 4 * N_stations]) && misfit[4 + 4 * N_stations] != 0.0)
        ++failures; // -5 vs -1 → match
    if (!std::isnan(misfit[4 + 0 * N_stations]) && misfit[4 + 0 * N_stations] != 1.0)
...
static int test_psr_hand_calculated() {
    ...
    // ---------- Launch ----------
    fm::launch_psr_kernel<Backend::OpenMP>(mt.data(), amp_P.data(), amp_S.data(), obs_psr.data(),
                                           misfit.data(), N_stations, N_trials);
...
static int test_combined_launch() {
    ...
    // ── Launch back-to-back ──
    fm::launch_polarity_kernel<Backend::OpenMP>(mt.data(), pol_vec.data(), obs_pol.data(),
                                                misfit_pol.data(), N_stations, N_trials);
    fm::launch_psr_kernel<Backend::OpenMP>(mt.data(), amp_P.data(), amp_S.data(), obs_psr.data(),
                                           misfit_psr.data(), N_stations, N_trials);
