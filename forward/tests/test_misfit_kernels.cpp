...
static int test_polarity_all_sign_combos() {
  ...
  const int N_trials =
      7; // dot: large+, small+, zero, small-, large-, NaN-prod, edge
  ...
  // Set up mt: each trial gives different Mxx (dot = Mxx since
  // pol_vec=[1,0,0,0,0,0]) Kernel uses mt[trial + comp * N_trials] (LayoutLeft
  // [N_trials × 6]) Trial 0: large positive (Mxx=+5.0) → syn_pol=+1 Trial 1:
  // small positive (Mxx=+0.001) → syn_pol=+1 Trial 2: zero (Mxx=0.0) →
  // syn_pol=0 Trial 3: small negative (Mxx=-0.001) → syn_pol=-1 Trial 4: large
  // negative (Mxx=-5.0) → syn_pol=-1 Trial 5: zero → syn_pol=0 Trial 6: large
  // positive → syn_pol=+1
  double mt_raw[] = {
  ...
  fm::launch_polarity_kernel<Backend::OpenMP>(mt.data(), pol_vec.data(),
                                              obs_pol.data(), misfit.data(),
                                              N_stations, N_trials);

  int failures = 0;

  // Station 0: expects +1 → dot+ matches (trial 0,1,6), mismatch (trial 3,4)
  assert(!std::isnan(misfit[0 + 0 * N_stations]) &&
         misfit[0 + 0 * N_stations] == 0.0); // +1 vs +1 → match
  assert(!std::isnan(misfit[0 + 1 * N_stations]) &&
         misfit[0 + 1 * N_stations] == 0.0); // small+ vs +1 → match
  if (!std::isnan(misfit[0 + 3 * N_stations]) &&
      misfit[0 + 3 * N_stations] != 1.0)
    ++failures; // -1 vs +1 → mismatch
  ...
  // Station 4: expects -1
  if (!std::isnan(misfit[4 + 4 * N_stations]) &&
      misfit[4 + 4 * N_stations] != 0.0)
    ++failures; // -5 vs -1 → match
  if (!std::isnan(misfit[4 + 0 * N_stations]) &&
      misfit[4 + 0 * N_stations] != 1.0)
    ++failures; // +5 vs -1 → mismatch
...
static int test_psr_hand_calculated() {
  ...
  for (int s = 0; s < N_stations; ++s) {
    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < 6; ++j) {
        amp_P[s + i * N_stations + j * (N_stations * 6)] =
            (i == j) ? ((i == 0) ? 4.0 : 1e-30) : 0.0;
        amp_S[s + i * N_stations + j * (N_stations * 6)] =
            (i == j) ? ((i == 0) ? 1.0 : 1e-30) : 0.0;
      }
    }
  }
  ...
  // ---------- Launch ----------
  fm::launch_psr_kernel<Backend::OpenMP>(mt.data(), amp_P.data(), amp_S.data(),
                                         obs_psr.data(), misfit.data(),
                                         N_stations, N_trials);
  ...
  for (int t = 0; t < N_trials; ++t) {
    if (!approx_eq(misfit[1 + t * N_stations], expected_mis)) {
      printf("  FAIL: station 1 trial %d: expected %.10g, got %.10g\n", t,
             expected_mis, misfit[1 + t * N_stations]);
      ++failures;
    }
  }
  ...
  for (int t = 0; t < N_trials; ++t) {
    if (!approx_eq(misfit[2 + t * N_stations], expected_mis2)) {
      printf("  FAIL: station 2 trial %d: expected %.10g, got %.10g\n", t,
             expected_mis2, misfit[2 + t * N_stations]);
      ++failures;
    }
  }
...
static int test_psr_nondiagonal() {
  ...
  for (int c = 0; c < 6; ++c)
    mt[1 + c * N_trials] = 0.0;
  mt[1 + 3 * N_trials] =
      1.0; // Mxy = 1 (index 3 in NED: [Mxx, Myy, Mzz, Mxy, Mxz, Myz])
  ...
  // Trial 1: Mxy=1, Mxx=0
  // quad_P = 0 (no contributing diagonal/cross terms for Mxy alone with this
  // amp_P) Actually: Mxx=0, Mxy=1 quad_P = Mxx²·4 + Mxx·Mxy·2 + Mxy·Mxx·2 = 0
...
static int test_polarity_edge_cases() {
  ...
  if (misfit[0 + 0 * N_stations] != 0.0) {
    printf("  FAIL: s0 t0 expected 0, got %g\n", misfit[0 + 0 * N_stations]);
    ++failures;
  }
  //   Trial 1: dot=-2 → syn=-1 → mismatch: 1.0
  if (misfit[0 + 1 * N_stations] != 1.0) {
    printf("  FAIL: s0 t1 expected 1, got %g\n", misfit[0 + 1 * N_stations]);
    ++failures;
  }

  // Station 1: all-zero pol_vec, obs=0 → NaN
  if (!std::isnan(misfit[1 + 0 * N_stations])) {
    printf("  FAIL: s1 t0 expected NaN\n");
    ++failures;
  }
  if (!std::isnan(misfit[1 + 1 * N_stations])) {
    printf("  FAIL: s1 t1 expected NaN\n");
    ++failures;
  }

  // Station 2: obs=-1, pol[0]=-3
  //   Trial 0: dot=-6 → syn=-1 → match: 0.0
  if (misfit[2 + 0 * N_stations] != 0.0) {
    printf("  FAIL: s2 t0 expected 0, got %g\n", misfit[2 + 0 * N_stations]);
    ++failures;
  }
  //   Trial 1: dot=6 → syn=+1 → mismatch: 1.0
  if (misfit[2 + 1 * N_stations] != 1.0) {
    printf("  FAIL: s2 t1 expected 1, got %g\n", misfit[2 + 1 * N_stations]);
    ++failures;
  }

  // Station 3: obs=-1, pol[0]=0.5
  //   Trial 0: Mxx=2 → dot=+1.0 → syn=+1 → mismatch
  if (misfit[3 + 0 * N_stations] != 1.0) {
    printf("  FAIL: s3 t0 expected 1, got %g\n", misfit[3 + 0 * N_stations]);
    ++failures;
  }
  //   Trial 1: Mxx=-2 → dot=-1.0 → syn=-1 → match
  if (misfit[3 + 1 * N_stations] != 0.0) {
    printf("  FAIL: s3 t1 expected 0, got %g\n", misfit[3 + 1 * N_stations]);
    ++failures;
  }

  // Station 4: obs=NaN → all NaN
  if (!std::isnan(misfit[4 + 0 * N_stations])) {
    printf("  FAIL: s4 t0 expected NaN\n");
    ++failures;
  }
  if (!std::isnan(misfit[4 + 1 * N_stations])) {
    printf("  FAIL: s4 t1 expected NaN\n");
    ++failures;
  }
...
static int test_psr_degenerate() {
  ...
  for (int t = 0; t < N_trials; ++t)
    for (int c = 0; c < 6; ++c)
      mt[t + c * N_trials] = 0.0;
  mt[1 + 0 * N_trials] =
      1e-20; // ultra-tiny (should still compute, not be degenerate)
...
static int test_combined_launch() {
  ...
  // ── Launch back-to-back ──
  fm::launch_polarity_kernel<Backend::OpenMP>(mt.data(), pol_vec.data(),
                                              obs_pol.data(), misfit_pol.data(),
                                              N_stations, N_trials);
  fm::launch_psr_kernel<Backend::OpenMP>(mt.data(), amp_P.data(), amp_S.data(),
                                         obs_psr.data(), misfit_psr.data(),
                                         N_stations, N_trials);
  ...
  if (misfit_pol[0 + 0 * N_stations] != 0.0) {
    printf("  FAIL: pol s0 t0\n");
    ++failures;
  }
  if (misfit_pol[0 + 1 * N_stations] != 0.0) {
    printf("  FAIL: pol s0 t1\n");
    ++failures;
  }
  if (misfit_pol[0 + 2 * N_stations] != 1.0) {
    printf("  FAIL: pol s0 t2\n");
    ++failures;
  }

  // Station 1: obs=-1 → syn=[+1,+1,-1] → [mismatch, mismatch, match]
  if (misfit_pol[1 + 0 * N_stations] != 1.0) {
    printf("  FAIL: pol s1 t0\n");
    ++failures;
  }
  if (misfit_pol[1 + 2 * N_stations] != 0.0) {
    printf("  FAIL: pol s1 t2\n");
    ++failures;
  }
