...
static std::vector<CsvRow> read_mt_csv(const std::string &path) {
    ...
    while (std::getline(in, line)) {
        ...
        CsvRow r;
        char comma;
        ss >> r.strike >> comma >> r.dip >> comma >> r.rake >> comma >> r.Mxx >> comma >> r.Myy >>
            comma >> r.Mzz >> comma >> r.Mxy >> comma >> r.Mxz >> comma >> r.Myz;
        rows.push_back(r);
    }
    return rows;
}
...
static int test_mt_csv(const char *csv_path) {
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
        const auto &r = rows[i];
        MomentTensor mt = sdr_to_mt(r.strike * DEG2RAD, r.dip * DEG2RAD, r.rake * DEG2RAD);
...
static int test_hdf5(const char *h5_path) {
    ...
    CHECK(dv1d.size() == 6 && std::abs(dv1d[0] - 1.1) < 1e-10 && std::abs(dv1d[5] - 6.6) < 1e-10,
          "double_array values");
    ...
    CHECK(shape_ok, "double_2d shape: " << rows << "×" << cols << " (expected 2×3 or 3×2)");
    // Verify 1.0 is first element; 6.0 is last element.
    CHECK(std::abs(dv2d.front() - 1.0) < 1e-10 && std::abs(dv2d.back() - 6.0) < 1e-10,
...
static int test_trials(const char *h5_path) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        CHECK(dip[i] >= 0.0 && dip[i] <= 90.0, "dip[" << i << "] = " << dip[i] << " in [0,90]");
    ...
    std::cout << "  First trial SDR=(" << s << "," << d << "," << r << ") → MT=(" << mt.Mxx << ","
              << mt.Myy << "," << mt.Mzz << ")" << std::endl;
