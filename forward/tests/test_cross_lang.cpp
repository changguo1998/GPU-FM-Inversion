...
static std::vector<CsvRow> read_mt_csv(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        CsvRow r;
        char comma;
        ss >> r.strike >> comma >> r.dip >> comma >> r.rake >> comma
           >> r.Mxx >> comma >> r.Myy >> comma >> r.Mzz >> comma
           >> r.Mxy >> comma >> r.Mxz >> comma >> r.Myz;
        rows.push_back(r);
    }
    return rows;
}
...
static int test_mt_csv(const char* csv_path) {
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
        const auto& r = rows[i];
        MomentTensor mt = sdr_to_mt(r.strike * DEG2RAD,
                                     r.dip    * DEG2RAD,
                                     r.rake   * DEG2RAD);

        double d = std::abs(mt.Mxx - r.Mxx); if (d > max_diff) max_diff = d;
        d = std::abs(mt.Myy - r.Myy);        if (d > max_diff) max_diff = d;
        d = std::abs(mt.Mzz - r.Mzz);        if (d > max_diff) max_diff = d;
        d = std::abs(mt.Mxy - r.Mxy);        if (d > max_diff) max_diff = d;
        d = std::abs(mt.Mxz - r.Mxz);        if (d > max_diff) max_diff = d;
        d = std::abs(mt.Myz - r.Myz);        if (d > max_diff) max_diff = d;
    }
...
static int test_trials(const char* h5_path) {
    ...
    for (int i = 0; i < n; ++i) {
        CHECK(freq_idx[i] >= 0, "freq_idx[" << i << "] = " << freq_idx[i] << " >= 0");
    }

    // Spot‑check MT for first trial
    double s = strike[0], d = dip[0], r = rake[0];
    MomentTensor mt = sdr_to_mt(s * M_PI / 180.0, d * M_PI / 180.0, r * M_PI / 180.0);
    std::cout << "  First trial SDR=(" << s << "," << d << "," << r
              << ") → MT=(" << mt.Mxx << "," << mt.Myy << "," << mt.Mzz << ")"
              << std::endl;
