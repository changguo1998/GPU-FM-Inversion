...
double ref_misfit(int phase, int trial, const double *mt, const double *cc_data,
                  const double *synamp_data, const double *obs_norm2, int Ntr, int Nph, int cc_pp) {
    double m[6];
    for (int c = 0; c < 6; ++c)
        m[c] = mt[trial + c * Ntr];
...
int main() {
    ...
    // ── Test 1: Hand-computed reference (2 phases, 3 trials, maxlag=1) ──
    {
        ...
        for (int p = 0; p < Nph; ++p)
            for (int i = 0; i < 6; ++i)
                for (int j = 0; j < 6; ++j)
                    h_syn[p + (i * 6 + j) * Nph] = (i == j) ? (1.0 + (rand() % 100) / 500.0)
        ...
        fm::launch_xcorr_misfit<Backend::OpenMP>(h_mt.data(), h_cc.data(), h_syn.data(),
                                                 h_obs.data(), h_mis.data(), Nph, Ntr, cc_pp);

        bool ok = true;
        double max_err = 0.0;
        for (int p = 0; p < Nph; ++p) {
            for (int t = 0; t < Ntr; ++t) {
                double expected = ref_misfit(p, t, h_mt.data(), h_cc.data(), h_syn.data(),
    ...
    //   Results captured for test 3.
    {
        ...
        // Reuse syn, m, lhs from test 2
        {
            ...
            fm::launch_xcorr_misfit<Backend::OpenMP>(g_mt.data(), g_cc.data(), g_syn.data(),
                                                     g_obs.data(), g_mis.data(), Nph, Ntr, cc_pp);
    ...
    // ── Test 4: Boundary — zero syn_norm2 (should return 1.0) ──
    {
        ...
        fm::launch_xcorr_misfit<Backend::OpenMP>(z_mt.data(), z_cc.data(), z_syn.data(),
                                                 z_obs.data(), z_mis.data(), Nph, Ntr, cc_pp);
    ...
    // ── Test 5: Boundary — single phase, single trial, maxlag=0 (cc_pp=1) ──
    {
        ...
        fm::launch_xcorr_misfit<Backend::OpenMP>(s_mt.data(), s_cc.data(), s_syn.data(),
                                                 s_obs.data(), s_mis.data(), Nph, Ntr, cc_pp);
