...
int main() {
    ...
    // ── Test 1: Hand-computed reference (2 phases, 3 trials, maxlag=1) ──
    {
        ...
        for (int t = 0; t < Ntr; ++t) {
            ...
            norm = std::sqrt(norm);
            for (int c = 0; c < 6; ++c) h_mt[t + c * Ntr] /= norm;
        }

        fm::launch_xcorr_misfit<Backend::OpenMP>(
            h_mt.data(), h_cc.data(), h_syn.data(),
            h_obs.data(), h_mis.data(),
            Nph, Ntr, cc_pp);
    ...
    //   Results captured for test 3.
    {
        ...
        // Reuse syn, m, lhs from test 2
        {
            ...
            for (int k = 0; k < 36; ++k) g_syn[0 + k * Nph] = syn[k];
            g_obs[0] = 1.0;
            for (int i = 0; i < 6; ++i) g_cc[0 + i * (Nph * cc_pp)] = 1.0;
            for (int i = 0; i < 6; ++i) g_mt[0 + i * Ntr] = m[i];

            fm::launch_xcorr_misfit<Backend::OpenMP>(
                g_mt.data(), g_cc.data(), g_syn.data(),
                g_obs.data(), g_mis.data(),
                Nph, Ntr, cc_pp);
    ...
    // ── Test 4: Boundary — zero syn_norm2 (should return 1.0) ──
    {
        ...
        for (int k = 0; k < 36; ++k) z_syn[0 + k * Nph] = 0.0;
        z_obs[0]    = 0.0;
        z_cc[0 + 0 * (Nph * cc_pp)] = 1.0;
        for (int i = 1; i < 6; ++i) z_cc[0 + i * (Nph * cc_pp)] = 0.0;
        for (int i = 0; i < 6; ++i) z_mt[0 + i * Ntr] = 0.0;

        fm::launch_xcorr_misfit<Backend::OpenMP>(
            z_mt.data(), z_cc.data(), z_syn.data(),
            z_obs.data(), z_mis.data(),
            Nph, Ntr, cc_pp);
    ...
    // ── Test 5: Boundary — single phase, single trial, maxlag=0 (cc_pp=1) ──
    {
        ...
        for (int k = 0; k < 6; ++k) s_mt[0 + k * Ntr] = 0.0;
        s_mt[0 + 0 * Ntr] = 1.0;

        s_cc[0 + 0 * (Nph * cc_pp)] = 1.0;
        for (int i = 1; i < 6; ++i) s_cc[0 + i * (Nph * cc_pp)] = 0.0;

        for (int k = 0; k < 36; ++k) s_syn[0 + k * Nph] = 0.0;
        s_syn[0 + 0 * Nph] = 1.0;

        s_obs[0] = 1.0;

        fm::launch_xcorr_misfit<Backend::OpenMP>(
            s_mt.data(), s_cc.data(), s_syn.data(),
            s_obs.data(), s_mis.data(),
            Nph, Ntr, cc_pp);
