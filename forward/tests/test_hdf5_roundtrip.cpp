...
static void write_scalar_int(hid_t file_id, const char *path, int value) {
    ...
    hid_t dset =
        H5Dcreate(file_id, path, H5T_NATIVE_INT, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
static void write_int_1d(hid_t file_id, const char *path, const int *data, hsize_t n) {
...
static void write_double_1d(hid_t file_id, const char *path, const double *data, hsize_t n) {
    hid_t space = H5Screate_simple(1, &n, nullptr);
    hid_t dset =
        H5Dcreate(file_id, path, H5T_NATIVE_DOUBLE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
int main() {
    ...
    // Round-trip: open and read back
    {
        ...
        CHECK(std::abs(dv - 3.14159) < 1e-10, "read_double_scalar: expected 3.14159, got " << dv);

        // 1D int array
        auto iv1d = h.read_int_1d("/int_array");
        CHECK(iv1d.size() == 5, "read_int_1d size: expected 5, got " << iv1d.size());
        if (iv1d.size() == 5) {
            CHECK(iv1d[0] == 10 && iv1d[1] == 20 && iv1d[2] == 30 && iv1d[3] == 40 && iv1d[4] == 50,
                  "read_int_1d values: [10,20,30,40,50]");
        }

        // 1D double array
        auto dv1d = h.read_double_1d("/double_array");
        CHECK(dv1d.size() == 6, "read_double_1d size: expected 6, got " << dv1d.size());
        if (dv1d.size() == 6) {
            CHECK(std::abs(dv1d[0] - 1.1) < 1e-10 && std::abs(dv1d[2] - 3.3) < 1e-10 &&
        ...
        if (rows == 2 && cols == 3) {
            CHECK(std::abs(dv2d[0] - 1.0) < 1e-10 && std::abs(dv2d[3] - 4.0) < 1e-10 &&
        ...
        CHECK(!h.group_exists("/nonexistent"), "group_exists('/nonexistent') false");
