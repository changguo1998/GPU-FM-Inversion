...
static void write_1d_double_dset(hid_t loc, const char *name, const double *data, hsize_t n) {
    hid_t space = H5Screate_simple(1, &n, nullptr);
    hid_t dset =
        H5Dcreate(loc, name, H5T_NATIVE_DOUBLE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    H5Dwrite(dset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
    H5Sclose(space);
    H5Dclose(dset);
}

static void write_2d_double_dset(hid_t loc, const char *name, const double *data, hsize_t r,
                                 hsize_t c) {
    hsize_t dims[2] = {r, c};
...
static void write_int_scalar(hid_t loc, const char *name, int value) {
    ...
    hid_t dset = H5Dcreate(loc, name, H5T_NATIVE_INT, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    H5Dwrite(dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &value);
    H5Sclose(space);
    H5Dclose(dset);
}

static void write_string_1d(hid_t loc, const char *name, const char *const *strings, hsize_t n) {
    hsize_t dims[1] = {n};
    hid_t space = H5Screate_simple(1, dims, nullptr);
    hid_t dtype = H5Tcopy(H5T_C_S1);
    H5Tset_size(dtype, H5T_VARIABLE);
    hid_t dset = H5Dcreate(loc, name, dtype, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
int main() {
    ...
    const int maxlag = 5;
    const int cc_rows = 2 * maxlag + 1; // 11
    ...
    // ── Build synthetic database.h5 ────────────────────────────────────
    {
        ...
        // Known test data: obs = [1, 2, 3], gf = [4,0,0,0,0,0, 5,0,0,0,0,0,
        // 6,0,0,0,0,0] N_samples = 3, only Mxx non-zero
        ...
        double obs_psr_P_val = 0.5, obs_psr_S_val = 0.3;
        write_1d_double_dset(phase_grp, "obs_psr", &obs_psr_P_val, 1);
        H5Gclose(phase_grp);
        ...
        write_2d_double_dset(phase_grp, "amp_P", amp, 6, 6);
        write_2d_double_dset(phase_grp, "amp_S", ampS, 6, 6);
        write_1d_double_dset(phase_grp, "obs_psr", &obs_psr_S_val, 1);
        H5Gclose(phase_grp);
        H5Gclose(psr_grp);
    ...
    // ── Test DataCache ─────────────────────────────────────────────────
    {
        ...
        // ── Verify XCorr cache ─────────────────────────────────────────
        // Direct access to host data (no mirror/deep_copy needed)
        const double *cc_h = entry->xcorr.cc;
        const double *synamp_h = entry->xcorr.synamp;
        const double *obs_norm2_h = entry->xcorr.obs_norm2;
        int n_phases_cc = entry->xcorr.n_phases;
        ...
        // synamp layout: [n_syn_phases × 6] column-major = synamp[phase*6+i +
        // j*n_syn_phases] synamp(0, 0) where synamp is [n_phases*6, 6] LayoutLeft
        // = synamp_h[0 + 0 * n_syn_phases] = synamp_h[0]
        CHECK_CLOSE(synamp_h[0], 77.0, 1e-10, "synamp[0] for phase 0 = 77");

        // Cross-correlation at lag=0, comp=0: sum(obs[t] * gf[t,0]) = 1*4 + 2*5 +
        // 3*6 = 32 cc layout: [n_phases*cc_pp × 6] column-major:
        // cc[(phase*cc_pp+lag) + comp*(n_phases*cc_pp)] n_phases_cc = 2, cc_pp = 11
        // cc_h(0 * cc_rows + maxlag, 0) = cc_h[maxlag + 0 * (2*11)] = cc_h[5]
        CHECK_CLOSE(cc_h[5], 32.0, 1e-10, "CC[lag=0, comp=0] phase 0 = 32");

        // ── Verify Polarity cache ───────────────────────────────────────
        const double *pol_vec_h = entry->polarity.pol_vec;
        const double *obs_pol_h = entry->polarity.obs_pol;
        int n_phases_pol = entry->polarity.n_phases;

        // pol_vec layout: [n_phases × 6] column-major: pol[phase + comp * n_phases]
        // pol_vec_h(0, 0) = pol_vec_h[0 + 0 * 2] = pol_vec_h[0] (value 3.0)
        // pol_vec_h(0, 1) = pol_vec_h[0 + 1 * 2] = pol_vec_h[2] (value 0.0)
        CHECK_CLOSE(pol_vec_h[0], 3.0, 1e-10, "pol_vec[0] phase 0 = 3");
        CHECK_CLOSE(pol_vec_h[2], 0.0, 1e-10, "pol_vec[2] phase 0 = 0");
        CHECK_CLOSE(obs_pol_h[0], 1.0, 1e-10, "obs_pol phase 0 = 1");
        CHECK_CLOSE(obs_pol_h[1], -1.0, 1e-10, "obs_pol phase 1 = -1");

        // ── Verify PSR cache ────────────────────────────────────────────
        const double *ampP_h = entry->psr.amp_P;
        const double *obs_psr_h = entry->psr.obs_psr;
        int n_phases_psr = entry->psr.n_phases;

        // amp_P layout: [n_phases × 6 × 6] column-major: amp[phase + i*n_phases +
        // j*(n_phases*6)] ampP_h(0, 0, 0) = ampP_h[0 + 0*n_phases_psr +
        // 0*(n_phases_psr*6)] = ampP_h[0]
        CHECK_CLOSE(ampP_h[0], 1.0, 1e-10, "amp_P[0] phase 0 = 1");
