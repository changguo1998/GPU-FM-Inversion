...
std::vector<std::string> DataCache::read_phase_ids(hid_t file_id,
                                                   const char *path) {
...
std::vector<int> DataCache::read_int_1d_direct(hid_t file_id,
                                               const char *path) {
  hid_t dset = H5Dopen(file_id, path, H5P_DEFAULT);
  if (dset < 0)
    throw std::runtime_error("Cannot open " + std::string(path));
...
CacheEntry DataCache::load_combo(const std::string &database_path, int freq_idx,
                                 int depth_idx,
  ...
  for (int i = 0; i < n_ph; ++i) {
    ...
    try {
      ...
      if (h5.group_exists(gf_pol_path.c_str())) {
        ...
        host_data[i].n_pol = r;
        host_data[i].obs_pol =
            h5.read_int_scalar((pol_path + "obs_pol").c_str());
        has_polarity = true;
      }
    ...
    try {
      ...
      if (h5.group_exists(ampP_path.c_str())) {
        ...
        host_data[i].ampS =
            h5.read_double_2d((psr_path + "amp_S").c_str(), r, c);
        host_data[i].obs_psr =
            h5.read_double_scalar((psr_path + "obs_psr").c_str());
        has_psr = true;
      }
    } catch (...) { /* module not present */
    }
  ...
  if (has_xcorr) {
    ...
    entry.xcorr.synamp =
        new double[static_cast<size_t>(entry.xcorr.n_syn_phases * 6)];
...
void DataCache::compute_polarity_reduction(
    CacheEntry & /*entry*/, const std::vector<double> & /*gf_pol*/,
    int /*n_pol_samples*/) {
  // See load_combo() — inline host-side reduction.
}

void DataCache::compute_psr_reduction(
    CacheEntry & /*entry*/, const std::vector<double> & /*ampP_host*/,
