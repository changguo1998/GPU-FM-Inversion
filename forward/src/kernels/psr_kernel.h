...
//   amp_P    [N_stations × 6 × 6]   amp_P[station + i * N_stations + j *
//   (N_stations * 6)] amp_S    [N_stations × 6 × 6]   same layout obs_psr
//   [N_stations]           obs_psr[station] misfit   [N_stations × N_trials]
//   misfit[station + trial * N_stations]
//
  ...
  Device<B>::parallel_for(N_stations * N_trials, [=](int idx) {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      for (int j = 0; j < 6; ++j) {
        ...
        amp_P_quad +=
            amp_P[station + i * N_stations + j * (N_stations * 6)] * m_i * m_j;
        amp_S_quad +=
            amp_S[station + i * N_stations + j * (N_stations * 6)] * m_i * m_j;
      }
    }
