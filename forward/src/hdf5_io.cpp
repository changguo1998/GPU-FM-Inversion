...
int Hdf5Handle::read_int_scalar(const char *path) {
  ...
  int value = 0;
  check_herr("H5Dread (int_scalar)", H5Dread(dset, H5T_NATIVE_INT, H5S_ALL,
...
std::vector<int> Hdf5Handle::read_int_1d(const char *path) {
  ...
  check_herr("H5Dread (int_1d)", H5Dread(dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
...
std::vector<double> Hdf5Handle::read_double_2d(const char *path, int &rows,
                                               int &cols) {
  hid_t dset = H5Dopen(file_id, path, H5P_DEFAULT);
