...
| `assess.jl` | Julia | Apply weights, aggregate, check convergence, update strategy → `status_{N+1}.h5` |
...
- **Misfit modules**: XCorr, Polarity, PSR, AbsShift, RelShift, CAP (DTW excluded)
...
2. `assess.jl` owns all strategy: weights, channel selection, convergence
3. All frequency-band variants precomputed upfront in `database.h5`
4. Misfits are unweighted: `[N_mod × N_phs × N_tr]`, weights applied in assess
